...
upgrade5 = function() {
  const btn = document.getElementById("upgrade5")
  const ltn = document.getElementById("pris5")
  if (teds >= pris5) {
    teds -= pris5
    multiplier *= 10
    multicounter *= 10
    ...
    if (ltn && ltn.parentNode) {
      ltn.parentNode.removeChild(ltn)
      remove2 = true
    }
    alert("NEGA TED UNLEASHED! Your multiplier is now 10x!")
    updateUI()
  }
}
function load(){
  ...
  if (remove2) {
    const ltn = document.getElementById("pris5")
    if (ltn && ltn.parentNode) ltn.parentNode.removeChild(ltn)
  }
...
function save(){
  if (resetting == false) {
    localStorage.setItem("userData",[tedsPerClick,tedsPerSecond,multiplier,superPerSecond,teds,k,m,s,st,pris,pris2,pris3,pris4,nega,remove,remove2])
  } 
}
setInterval(() => {
    ...
    if (nega == true) {
      document.getElementById("ted").src = "nega.jpg"
